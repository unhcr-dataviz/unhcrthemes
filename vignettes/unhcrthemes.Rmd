...
The `grid` parameter control the display of both major and minor grid lines in your plot. By default, major grid lines are visible and minor hiddend.

Using `grid = "XY"` adds both major X and Y grid lines, `grid = "X"` adds only major X grid lines, `grid = "Y"` adds only major Y grid lines, and `grid = FALSE` removes all grid lines. You can minor grid lines by using lowercase `"xy"`, `"x"` or `"y"`. For example, let's keep only the major Y grid lines:
...
The axis represents of course the axis lines but it also includes the different axis elements such as the axis title, axis text, and axis ticks.
...
In this example, we will set the `expand` argument to `expansion(c(0, 0.01))` for the y axis to remove the unwanted space, effectively "expanding" the axis by removing the space at the axis origins and adding a small padding at the other ends.
...
Text labels added with `geom_text` will also come with the recommended font and style. 
...
For more information on each parameter and usage, visit the [function documentation page](https://unhcr-dataviz.github.io/unhcrthemes/reference/index.html) and the [UNHCR Datavisualization platform](https://dataviz.unhcr.org/tools/r/)
