...
Let's create a 100% stacked column chart of the last 10 years of total refugee population originated by Africa's region.
...
ggplot(
  ...
  treemapify::geom_treemap_text(
    aes(label = paste0(
      unhcr_region, "\n",
      scales::label_number(scale_cut = scales::cut_short_scale())(refugees)
    )),
    color = "#FFFFFF",
    size = 9,
    lineheight = 1.1,
...
ggplot(
  ...
  theme_unhcr(axis_title = "y")
```

### Sankey diagram

Let's create a Sankey diagram showing the refugee population by region of origin and destination in 2022. For this we will use the [`{ggalluvial}`](https://corybrunson.github.io/ggalluvial/) package.

**Preparing the data:**

```{r sankey-data}
# Select 2022 data, Load and join region names for both origin and asylum,
# Group Sub-Saharan Africa regions togetherfor both origin and asylum,
# Group by coo_region and coa_region, Sum refugees, make region factors
sankey_df <-
  refugees::population |>
  filter(year == 2022) |>
  left_join(refugees::countries, by = c("coo_iso" = "iso_code")) |>
  select(coa_iso, origin = unhcr_region, refugees) |>
  left_join(refugees::countries, by = c("coa_iso" = "iso_code")) |>
  select(origin, destination = unhcr_region, refugees) |>
  mutate(
    origin = case_when(
      origin %in% c(
        "East and Horn of Africa",
    ...
    destination = case_when(
      ...
      destination == "The Americas" ~ "Americas",
      TRUE ~ destination
    )
  ) |>
...
```{r sankey-plot, out.width="100%"}
# install.packages("ggalluvial")
library(ggalluvial)

# Set title
sankey_title <- "Refugees from their region of origin to their region of asylum  | 2022"


# Plot
ggplot(sankey_df, aes(
  axis1 = origin,
  axis2 = destination,
  y = refugees
)) +
