...
```{r bar1, fig.width = 7, fig.height = 4.25}
df1 <- tribble(
~year, ~nidp,
"2010",24.98,
"2011",22.44,
...
```{r bar1_alt, fig.width = 7, fig.height = 5}
ggplot(df1) +
  geom_col(aes(x = year, y = nidp),
           fill = unhcr_pal(n = 1, "pal_blue"),
           width = 0.8) +
...
```{r col1, fig.width = 7, fig.height = 6}
df2 <- tribble(
  ~country,~displaced,
  "Eritrea",522000,
...
```{r col1_alt, fig.width = 7, fig.height = 6}
df2 |>
  ggplot() +
  geom_col(aes(x = displaced/1e6,
               y = reorder(country, displaced)),
...
```{r bar2, fig.width = 7, fig.height = 5}
df3 <- tribble(
...
```{r bar2_alt, fig.width = 7, fig.height = 5}
df3 |>
  pivot_longer(cols = -year) |>
  ggplot() +
...
```{r col2, fig.width = 7, fig.height = 6}
df4 <- tribble(
  ~region,~'2019',~'2020',
...
```{r col2_alt, fig.width = 7, fig.height = 6}
df4 |>
  pivot_longer(cols = -region) |>
  ggplot() +
  geom_col(aes(value/1e3, fct_rev(factor(region)), fill = name),
...
```{r bar3, fig.width = 7, fig.height = 5}
df5 <- tribble(
...
```{r bar3_alt, fig.width = 7, fig.height = 5}
df5 |>
  pivot_longer(cols = -Year) |>
  ggplot() +
  geom_col(aes(x = Year, y = value, fill = name),
...
```{r bar4, fig.width = 7, fig.height = 6}
df6 <- tribble(
...
```{r bar4_alt, fig.width = 7, fig.height = 6}
df6_long <- df6 |>
  pivot_longer(cols = -Country)
...
```{r bar5, fig.width = 7, fig.height = 5}
df7 <- tribble(
...
```{r bar6, fig.width = 7, fig.height = 6}
df8 <- tribble(
  ~ages,~male,~female,
  "0-4",0.06,0.06,
  "5-11",0.09,0.08,
...
```{r area, fig.width = 7, fig.height = 5}
df10 <- tribble(
...
```{r line2, fig.width = 7, fig.height = 5}
df11 <- tribble(
...
```{r treemap, fig.width = 7, fig.height = 6}
df13 <- tribble(
  ~region,~workforce,
...
```{r pie, fig.width = 7, fig.height = 6}
df12 <- tribble(
  ~label,~Funding,
...
```{r donut, fig.width = 7, fig.height = 6}
df12 |>
  mutate(p = Funding / sum(Funding),
