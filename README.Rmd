...
For R PDF and PostScript devices, there is a function `import_lato` that can be used to import the `Lato` font into the device. We recommend the use of `cairo_pdf` or `cairo_ps` devices, which are available on all platforms and better support custom fonts.
